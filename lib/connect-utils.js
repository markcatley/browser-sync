...
var connectUtils = {
    ...
    scriptTags: function (options) {
        ...
        if (script.match(/HOST/)) {
            template = fs.readFileSync(config.templates.scriptTagWithHostReplacement, "utf-8");
        } else {
            template = fs.readFileSync(config.templates.scriptTagWithoutHostReplacement, "utf-8");
        }
        template = template
